...
const RISK_CONFIG = {
    Low: { bg: 'bg-emerald-500/15', border: 'border-emerald-500/30', text: 'text-emerald-400', dot: 'bg-emerald-400', glow: 'risk-glow-low' },
    Medium: { bg: 'bg-amber-500/15', border: 'border-amber-500/30', text: 'text-amber-400', dot: 'bg-amber-400', glow: 'risk-glow-medium' },
    High: { bg: 'bg-red-500/15', border: 'border-red-500/30', text: 'text-red-400', dot: 'bg-red-400', glow: 'risk-glow-high' },
}
...
export default function Result() {
    ...
    // Animated bar widths: start at 0, animate to actual values after mount
            ...
            <div className="min-h-screen bg-slate-950">
                ...
                <header className="border-b border-slate-800 px-6 py-4 flex items-center justify-between sticky top-0 bg-slate-950/90 backdrop-blur-md z-20">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-xs text-slate-500 bg-slate-800 border border-slate-700 px-3 py-1.5 rounded-full font-medium">
                ...
                <div className="max-w-5xl mx-auto px-4 py-8">
                    <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
                        <div>
                            ...
                            <p className="text-slate-400 text-sm">Analysis complete · Fusion model applied</p>
                        </div>
                        {/* Decision badge */}
                        <div className={`flex items-center gap-2.5 px-5 py-2.5 rounded-2xl text-white font-bold text-sm shadow-lg ${decisionCfg.bg}`}>
                            <span className="text-base">{decisionCfg.icon}</span>
                            Decision: {decisionCfg.label}
                        </div>
                    </div>

                    {/* Top row: Gauge + Risk + Mini stats */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
                        ...
                        <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div className="card p-5 flex flex-col justify-between">
                                ...
                                <div>
                                    <p className={`text-5xl font-extrabold ${riskCfg.text}`}>{trustPct}<span className="text-2xl font-bold text-slate-500">%</span></p>
                                    <p className="text-xs text-slate-500 mt-1">Fused risk index</p>
                                </div>
                                <div className="mt-4 bg-slate-800 rounded-full h-2 overflow-hidden">
                                    <div
                                        className={`h-full rounded-full progress-bar ${trustBarColor}`}
                                        style={{ width: `${barWidths.trust}%` }}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="card p-5">
                                ...
                                <p className="text-4xl font-extrabold text-blue-400">
                                    {Math.round(credit_score * 100)}<span className="text-xl font-bold text-slate-500">%</span>
                                </p>
                                <p className="text-xs text-slate-500 mt-1 mb-3">Raw credit signal</p>
                                <div className="bg-slate-800 rounded-full h-1.5 overflow-hidden">
                                    <div className="h-full rounded-full bg-blue-500 progress-bar" style={{ width: `${barWidths.credit}%` }} />
                                </div>
                            </div>
                            ...
                            <div className="card p-5">
                                ...
                                <p className="text-4xl font-extrabold text-rose-400">
                                    {Math.round(fraud_score * 100)}<span className="text-xl font-bold text-slate-500">%</span>
                                </p>
                                <p className="text-xs text-slate-500 mt-1 mb-3">Raw fraud signal</p>
                                <div className="bg-slate-800 rounded-full h-1.5 overflow-hidden">
                                    <div className="h-full rounded-full bg-rose-500 progress-bar" style={{ width: `${barWidths.fraud}%` }} />
                                </div>
                            </div>
                            ...
                            <div className="card p-5">
                                ...
                                <div className="flex flex-col gap-2 mt-1">
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs text-blue-400 font-semibold">Credit</span>
                                        <span className="text-xs text-blue-400 font-bold">{credit_contribution}%</span>
                                    </div>
                                    <div className="bg-slate-800 rounded-full h-1.5 overflow-hidden">
                                        <div className="h-full rounded-full bg-blue-500 progress-bar" style={{ width: `${barWidths.creditWeight}%` }} />
                                    </div>
                                    <div className="flex items-center justify-between mt-1">
                                        <span className="text-xs text-rose-400 font-semibold">Fraud</span>
                                        <span className="text-xs text-rose-400 font-bold">{fraud_contribution}%</span>
                                    </div>
                                    <div className="bg-slate-800 rounded-full h-1.5 overflow-hidden">
                                        <div className="h-full rounded-full bg-rose-500 progress-bar" style={{ width: `${barWidths.fraudWeight}%` }} />
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {/* Bottom row: Pie + Decision detail */}
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-5">
                        ...
                        <div className="md:col-span-3 card p-6 flex flex-col justify-between">
                            <div>
                                ...
                                <div className={`flex items-start gap-4 p-4 rounded-xl border ${riskCfg.bg} ${riskCfg.border}`}>
                                    <div className={`shrink-0 w-10 h-10 rounded-xl flex items-center justify-center font-bold text-xl ${decisionCfg.bg} text-white shadow-lg`}>
                                        {decisionCfg.icon}
                                    </div>
                                    <div>
